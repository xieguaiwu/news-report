...
## Corpus Check
- 87 files · ~62,823 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 841 nodes · 1520 edges · 50 communities (37 shown, 13 thin omitted)
...
### Community 0 - "fetch.go"
Cohesion: 0.09
Nodes (28): Options, robotsRule, StatusError, globToRegexp(), Context, Duration, Time, URL (+20 more)
...
### Community 3 - "Run"
Cohesion: 0.11
Nodes (39): buildSources(), collectSource(), Cache, Context, Duration, Time, Run(), sourceWeight() (+31 more)
...
### Community 16 - "tui.go"
Cohesion: 0.06
Nodes (51): Builder, Category, Config, LLMConfig, SourceOverride, Fetcher, expandPath(), Duration (+43 more)
...
Cohesion: 0.07
Nodes (48): buildArticleMsg(), Context, Client, parseBriefResult(), parseNumberedLines(), CacheKey(), Duration, RWMutex (+40 more)
...
- **Why does `Fetcher` connect `tui.go` to `fetch.go`, `Search`, `Run`, `Extract`?**
  _High betweenness centrality (0.160) - this node is a cross-community bridge._
- **Why does `CacheKey()` connect `llm_test.go` to `tui.go`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `Run()` connect `Run` to `tui.go`, `Score`, `IsDuplicate`, `Classify`?**
...
- **Should `fetch.go` be split into smaller, more focused modules?**
